   ...
   if ( ! $a->test_MarketDate("SYMBOL"	=> $ref->{symbol},
      ...
      if ( $#lines eq 0 ) {
         ...
         for ( $i = $#DatHash ; $i ge 0 ; $i-=1 ) {
            eval { $a->{"insert_daily"}->execute($ref->{idx},$DatHash[$i]{Open},
		$DatHash[$i]{Hi},$DatHash[$i]{Lo},$DatHash[$i]{Close},
      ...
      print SF $timestamp . ": Selective Update, data already exists for $ref->{symbol}\n";
      }
   }
